...
class FileValidator:
    ...
    @classmethod
    def validate_file_type(cls, file_path: str, content_type: str = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if content_type:
                ...
                if extension not in cls.SUPPORTED_TYPES[content_type]:
                    return False, f"File extension {extension} doesn't match MIME type {content_type}"
            
            # Use python-magic to detect actual file type
            try:
                detected_type = magic.from_file(str(file_path), mime=True)
                if detected_type not in cls.SUPPORTED_TYPES:
                    return False, f"File appears to be {detected_type}, not a supported resume format"
            except Exception:
    ...
    def validate_file_size(cls, file_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if file_size < cls.MIN_FILE_SIZE:
                return False, f"File too small: {file_size} bytes (minimum: {cls.MIN_FILE_SIZE} bytes)"
            
            if file_size > cls.MAX_FILE_SIZE:
                return False, f"File too large: {file_size} bytes (maximum: {cls.MAX_FILE_SIZE} bytes)"
...
class DataValidator:
    ...
    def validate_contact_info(contact_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results['completeness_score'] = (len(required_fields) + present_fields) / (len(required_fields) + len(optional_fields)) * 100
        ...
        for i, exp in enumerate(experience):
            ...
            if exp.get('start_date') and exp.get('end_date'):
                try:
                    ...
                    if start > end:
                        results['warnings'].append(f"Experience {i+1}: Start date after end date")
                except ValueError:
...
class JobDescriptionValidator:
    ...
    def validate_job_description(cls, job_description: str) -> Dict[str, Any]:
        ...
        if len(job_description) < cls.MIN_LENGTH:
            results['errors'].append(f"Job description too short (minimum {cls.MIN_LENGTH} characters)")
            results['is_valid'] = False
        
        if len(job_description) > cls.MAX_LENGTH:
            results['errors'].append(f"Job description too long (maximum {cls.MAX_LENGTH} characters)")
            results['is_valid'] = False
        ...
        common_elements = ['requirements', 'responsibilities', 'qualifications', 'experience', 'skills']
        found_elements = [elem for elem in common_elements if elem.lower() in job_description.lower()]
...
class AnalysisValidator:
    ...
    def validate_analysis_result(result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(ats_score, (int, float)) or ats_score < 0 or ats_score > 100:
            results['warnings'].append("ATS score appears to be out of valid range (0-100)")
