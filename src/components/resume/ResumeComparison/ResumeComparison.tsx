...
export const ResumeComparison: React.FC<ResumeComparisonProps> = ({
  ...
  const [availableJobCategories, setAvailableJobCategories] = useState<
    string[]
  >([]);
  ...
  const filteredVariants = selectedJobCategory
    ? variants.filter(v =>
        v.analyses.some(a => a.jobCategory === selectedJobCategory)
      )
    : variants;
  ...
  const sortedVariants = filteredVariants.sort((a, b) => {
    if (selectedJobCategory) {
      const aScore =
        a.analyses.find(
          analysis => analysis.jobCategory === selectedJobCategory
        )?.atsScore || 0;
      const bScore =
        b.analyses.find(
          analysis => analysis.jobCategory === selectedJobCategory
    ...
    <div className='space-y-6'>
      ...
      {sortedVariants.length === 0 ? (
        <div className='text-center py-8 text-gray-500 dark:text-gray-400'>
          ...
          {selectedJobCategory && (
            <p className='text-sm'>
              No resumes have been analyzed for {selectedJobCategory}
            </p>
          )}
        </div>
      ) : (
            ...
            <tbody>
              {sortedVariants.map((variant, index) => {
                const relevantAnalysis = selectedJobCategory
                  ? variant.analyses.find(
                      a => a.jobCategory === selectedJobCategory
                    )
                  : variant.analyses.reduce(
                      (best, current) =>
      ...
      {sortedVariants.length > 0 && (
        <div className='bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4'>
          ...
          <p className='text-blue-800 dark:text-blue-200'>
            {selectedJobCategory ? (
              <>
                Best resume for <strong>{selectedJobCategory}</strong>:{' '}
                <strong>{sortedVariants[0]?.name}</strong>
                {sortedVariants[0]?.analyses.find(
                  a => a.jobCategory === selectedJobCategory
                )?.atsScore &&
              ...
              <>
                Your strongest resume overall:{' '}
                <strong>{sortedVariants[0]?.name}</strong>
                {sortedVariants[0]?.bestScore &&
                  ` (${sortedVariants[0].bestScore}% best score)`}
              </>
            )}
          </p>
        </div>
