...
class FileParser:
            ...
            for page_num in range(len(doc)):
                ...
                if self._detect_formatting_issues(page_text):
                    formatting_issues.append(
                        f"Page {page_num + 1}: Potential formatting issues detected"
                    )

            # Analyze formatting issues
            if images_count > 0:
                formatting_issues.append(
                    f"Contains {images_count} image(s) - may cause ATS parsing issues"
                )

            if tables_detected:
                formatting_issues.append(
                    "Tables detected - ensure content is also in plain text"
                )

            if len(fonts_used) > 3:
                formatting_issues.append(
                    f"Multiple fonts detected ({len(fonts_used)}) - stick to 1-2 standard fonts"
                )

            # Check for unusual fonts
            common_fonts = ["Times", "Arial", "Helvetica", "Calibri", "Georgia"]
            unusual_fonts = [
                f for f in fonts_used if not any(cf in f for cf in common_fonts)
            ]
            if unusual_fonts:
                formatting_issues.append(
                    f"Unusual fonts detected: {', '.join(list(unusual_fonts)[:3])}"
                )
    ...
    def _parse_docx_enhanced(self, file_content: bytes) -> dict[str, Any]:
        ...
        try:
            ...
            if images_count > 0:
                formatting_issues.append(
                    f"Contains {images_count} image(s) - may cause ATS issues"
                )

            if tables_count > 2:
                formatting_issues.append(
                    f"Contains {tables_count} tables - consider simplifying layout"
                )
    ...
    def _parse_txt(self, file_content: bytes) -> dict[str, Any]:
        ...
        try:
            ...
            raise Exception(f"Error parsing TXT: {e!s}")
